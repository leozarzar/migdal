...
const Dashboard = {
    ...
    async _computeGroupItemKpis(memberNames, policyItem, policyData) {
        ...
        // Lead time: per-item override → average of members → policy default
        let lt;
        if (leadTimeType === "custom") {
            ...
            const ltValues = await Promise.all(memberNames.map(n => this._fetchLeadTime(n)));
        ...
        try {
            const mkQuery = name => new URLSearchParams({ material: name, startDate, endDate });
            ...
            // Merge buckets by summing across all members
            const mergedMap = new Map();
            memberNames.forEach((_, idx) => {
                const buckets = this._balanceAggregate(
                    allConsumption[idx] || [], aggregation, startDate, allStock[idx] || []
                );
                buckets.forEach(b => {
            ...
            const filtered = this._balanceFilter(merged, removeZeros, treatOutliers, treatRuptures);
    ...
    async _computeItemKpis(materialName, policyItem, policyData) {
        ...
        if (leadTimeType === "custom") {
            ...
            lt = Math.max((await this._fetchLeadTime(materialName)) ?? 1, 1);
        }
        ...
        try {
            ...
            const aggregated = this._balanceAggregate(consumptionRows || [], aggregation, startDate, stockRows || []);
            const filtered   = this._balanceFilter(aggregated, removeZeros, treatOutliers, treatRuptures);
            if (filtered.length < 2) return { safetyStock: null, reorderPoint: null, maxStock: null };
    ...
    /**
     * Agrega consumo diário em buckets (diario/semanal/mensal).
     * Preenche gaps com zero e marca períodos com estoque positivo.
     */
    ...
    /**
     * Filtra série temporal: remove zeros, rupturas (sem estoque) e outliers via IQR.
     */
    _balanceFilter(data, removeZeros, treatOutliers, treatRuptures) {
        let result = data.slice();
        if (removeZeros)   result = result.filter(d => d.value > 0);
        // Rupturas: mantém apenas períodos com consumo ou com estoque registrado
        if (treatRuptures) result = result.filter(d => d.value > 0 || d.hasStock);
        if (treatOutliers && result.length >= 4) {
            // Filtragem por IQR: descarta valores fora de [Q1 - 1.5×IQR, Q3 + 1.5×IQR]
    ...
    /**
     * Aplica o modelo de forecast escolhido e retorna previsão + desvio padrão.
     * Métodos: média móvel, suavização exponencial, regressão linear, média simples.
    ...
    _balanceForecast(data, method, params, serviceLevel) {
        ...
        if (method === "moving-average") {
            // Média móvel de p períodos: forecast = média dos últimos p valores
            ...
            // Suavização exponencial simples: S_t = α·Y_t + (1−α)·S_{t−1}
            ...
            // Regressão linear: y = a + b·x, projeta próximo período
            ...
            // Fallback: média simples acumulada
            for (let i = 1; i < n; i++) {
        ...
        // Desvio padrão dos resíduos (RMSE) — mede precisão do modelo
        const std = residuals.length
            ? Math.sqrt(residuals.reduce((s, v) => s + v * v, 0) / residuals.length)
            : 0;
        return { nextForecast, std, z: this._balanceZScore(serviceLevel) };
    },

    /**
     * Regressão linear simples: calcula coeficientes a (intercepto) e b (inclinação).
     * Fórmula: b = (n∑xy − ∑x∑y) / (n∑x² − (∑x)²), a = (ȳ − b·x̄)
     */
    ...
    /**
     * Aproximação racional do Z-score (inversa da normal padrão).
     * Usa fórmula de Abramowitz & Stegun (equação 26.2.23).
    ...
    _balanceZScore(serviceLevel) {
        ...
        // t = √(−2·ln(1−p)); aproximação polinomial racional para a inversa da CDF normal
