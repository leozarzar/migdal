...
Object.assign(MobApp, {
    ...
    HomeScreen: {
        ...
        async _computeItemKpis(materialName, policyItem, policyData) {
            ...
            let lt;
            if (leadTimeType === 'custom')         lt = customLT;
            else if (policyItem.lead_time_days)    lt = policyItem.lead_time_days;
            else lt = Math.max((await this._fetchLeadTime(materialName)) ?? 1, 1);
            ...
            try {
                ...
                const fc = this._balanceForecast(filtered, model, { period: param, alpha: param, regressionPeriod: param }, serviceLevel);
                ...
                return {
                    ...
                    maxStock:     reviewType === 'periodic' ? Math.max(0, dDaily * (reviewPeriodDays + lt) + safetyStock) : null,
                };
            } catch {
                return { safetyStock: null, reorderPoint: null, maxStock: null };
            }
        ...
        async _computeGroupItemKpis(memberNames, policyItem, policyData) {
            ...
            try {
                ...
                memberNames.forEach((_, idx) => {
                    const buckets = this._balanceAggregate(allConsumption[idx] || [], aggregation, startDate, allStock[idx] || []);
                    buckets.forEach(b => {
